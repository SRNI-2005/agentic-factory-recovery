...
Then apply the `frontend-design` skill process + impeccable's two-altitude category-reflex check: the uupm first hit (dark slate + green accent, "Real-Time Operations") is a listed first-order reflex for observability tools. Rework until neither theme+palette nor theme-minus-one-accent is guessable from the category. Draw from SCADA/annunciator vernacular — e.g., warm phosphor amber on deep blue-black panel tones with engraved-label typography, or light engineering-drawing style with blueprint linework — but CHOOSE, don't take this suggestion as the default either. DESIGN.md must contain: 4–6 named hex values, display/body/utility typefaces (not Inter), layout concept + ASCII wireframe of CockpitView, signature element, and an explicit anti-reference list.
