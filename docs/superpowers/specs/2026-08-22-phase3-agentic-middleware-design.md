...
> **Amendment 2026-08-25 (user-approved): time-phased shortfall warnings restore DEFER reachability.** Phase 2's MATERIAL_SHORTFALL (its 2026-08-25 amendment) may now fire even when grand totals suffice — whenever cumulative demand released before some receipt exceeds stock plus timely arrivals (a timing-only deficit). This restores step 3 below exactly as authored: "only timing is wrong" presupposes a warning that survives sufficient totals, and DEFER_JOB covers precisely that case. Exhaustion and back-edge semantics (§3.1) are unchanged.
